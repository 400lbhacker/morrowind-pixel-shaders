// Joseph Erickson Pixel Shader p1
// Donations for Grief potions welcome: https://paypal.me/JosephE741?country.x=US&locale.x=en_US
...
float4 combine(float2 tex : TEXCOORD0) : COLOR
{
    ...
    // Apply Van Gogh painting effect
    result *= 0.8; // Darken the image
    result = saturate(result * 1.5); // Increase overall intensity
    result = pow(result, 0.7); // Apply gamma correction for richness
    result = result * (1 - final); // Remove the purplish tone
    result = result * 1.5 - 0.3; // Increase overall brightness and contrast
